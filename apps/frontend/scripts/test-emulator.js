...
const localFirebaseBin = path.resolve(
  backendPath,
...
if (!fs.existsSync(localFirebaseBin)) {
  ...
  console.error(
    "Please install it by running: cd ../backend && npm install --save-dev firebase-tools\n",
  );
  process.exit(1);
}
...
const projectId =
  process.env.NEXT_PUBLIC_FIREBASE_PROJECT_ID || "buddy-connect";
...
const testEmulatorConnection = async () => {
  try {
    ...
    try {
      ...
      console.log(
        `✅ Health check successful! Status: ${healthResponse.status}`,
      );
      console.log(
      ...
      try {
        ...
        console.log(
          `Response: ${JSON.stringify(helloResponse.data, null, 2)}\n`,
        );

        console.log(
          "⚠️ Your hello function works, but the main API function is not available.",
        );
        console.log(
          "This suggests an issue with loading your Express app in Firebase Functions.",
        );
      } catch (helloError) {
        ...
        if (helloError.response) {
          ...
          console.error(
            `Data: ${JSON.stringify(helloError.response.data, null, 2)}`,
          );
        }
        ...
        console.error(
          "2. Verify your index.ts is exporting functions correctly",
        );
        console.error(
          "3. Make sure firebase.json points to the correct source directory",
        );
    ...
    console.log(
      "⚠️ Note: API endpoints requiring authentication will fail without a valid token.",
    );
    console.log(
      "👉 Use the web app with emulator mode enabled to test authenticated endpoints.\n",
    );
