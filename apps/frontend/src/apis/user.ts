...
export const updateUserData = async (userId: string = 'me', userData: Partial<User>) => {
  const path = getPath(`/users/${userId}`);
  return api.put<{ success: boolean; data: User; message: string }>(path, userData);
};
...
export const configureApiForEmulator = () => {
  ...
  const projectId = process.env.NEXT_PUBLIC_FIREBASE_PROJECT_ID || 'buddy-connect-f1215';
  ...
  console.log(`API now configured to use Firebase Functions emulator at: ${emulatorBaseUrl}`);
  console.log(`Register endpoint will now be at: ${emulatorBaseUrl}/api/auth/register`);
  
  // Test the connection to the emulator
  const testUrl = `${emulatorBaseUrl}/hello`;
  axios.get(testUrl)
    .then(response => {
      console.log('✅ Connected to Firebase Emulator successfully:', response.data);
    })
    .catch(error => {
      console.warn('⚠️ Could not connect to Firebase Emulator hello function:', error.message);
      console.warn('API calls may fail if the Firebase Emulator is not running properly.');
    });
  ...
  api.interceptors.request.use(request => {
    if (request.url) {
      ...
      console.log(`Emulator Request: ${request.method?.toUpperCase()} ${fullUrl}`);
    } else {
      console.log(`Emulator Request: ${request.method?.toUpperCase()} ${request.url}`);
    }
    return request;
  });
