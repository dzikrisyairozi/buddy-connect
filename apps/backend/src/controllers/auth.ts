  ...
  try {
    ...
    try {
      ...
      if (
        ...
        createError.message.includes("Firestore")
      ) {
        try {
          ...
          console.log(
            `Deleted Firebase Auth user ${authUser.uid} due to Firestore creation failure`,
          );
        } catch (cleanupError) {
          console.error(
            "Failed to clean up Firebase Auth user after Firestore error:",
            cleanupError,
          );
        }
      }
